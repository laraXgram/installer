...
//            ->addOption('dev', null, InputOption::VALUE_NONE, 'Install the latest "development" release')
            ...
            ->addOption('surge', null, InputOption::VALUE_REQUIRED, 'Add LaraGram Surge to the project')
...
FLAG
        ...
        if (! $this->usingStarterKit($input)) {
            match (select(
                ...
                options: [
                    'none' => 'None',
                    'surge' => 'LaraGram Surge',
                ],
                default: 'none',
            )) {
                'surge' => $input->setOption('surge', true),
                default => null,
            };
        }
    }
    ...
    /**
     * Determine if the application is using LaraGram 11 or newer.
     *
     * @param  string  $directory
     * @return bool
     */
    public function usingLaraGramVersionOrNewer(int $usingVersion, string $directory): bool
    {
        $version = json_decode(file_get_contents($directory.'/composer.json'), true)['require']['laragram/core'];
    ...
    /**
     * Create a Git repository and commit the base LaraGram skeleton.
     *
     * @param  string  $directory
     * @param  \LaraGram\Console\Input\InputInterface  $input
     * @param  \LaraGram\Console\Output\OutputInterface  $output
    ...
    {
        return match (true) {
            $input->getOption('surge') => 'laraxgram/surge',
            default => $input->getOption('using'),
        };
    }

    /**
     * Determine if a LaraGram first-party starter kit has been chosen.
     *
     * @param  \LaraGram\Console\Input\InputInterface  $input
     * @return bool
     */
    protected function usingLaraGramStarterKit(InputInterface $input): bool
    {
        return $this->usingStarterKit($input) &&
               str_starts_with($this->getStarterKit($input), 'laragram/');
    }
